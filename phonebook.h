...
repeat:
	...
	void pub :: newrecord() {
		...
		strcpy(eml,email);
		fstream file;
		file.open("public.txt", ios::in | ios::app | ios::binary );
		...
		if(found==1)
			...
			file.open("public.txt", ios::app | ios::binary );
	...
	void pub :: display() {
		//Display All Records
		fstream file;
		file.open("public.txt", ios::ate | ios::in | ios::out | ios::binary );
	...
	void pub :: deleterec() {
		...
		cin>>bb;
		fstream file;
		file.open("public.txt",  ios::in | ios::binary );
		fstream fout;
		...
		if(i-j<=1) {
			...
			file.close();
			remove("public.txt");
			rename("tempfileph.txt","public.txt");
			if(j<i)cout<<"\n\nRECORD DELETED\n\n"<<endl;
			else cout<<"Record NOT found!!"<<endl;
		}
	}
	void pub :: showemergency() {
...
void pvt :: newrecord() {
	...
	strcpy(eml,email);
	fstream file;
	file.open("private.txt", ios::in | ios::app | ios::binary );
	while(file.read((char *) this, sizeof(*this))) {
		if(strcmp(rrep,getphno())==0) {
			found=1;
			break;
	...
	if(found==1)
		...
		file.open("private.txt", ios::app | ios::binary );
		update(nmopi,rrep,eml);
		file.write((char *) this, sizeof(*this));
		cout<<"Record Added Succesfully!!\n";
		file.close();
...
void pvt :: display() {
	fstream file;
	file.open("private.txt", ios::ate | ios::in | ios::out | ios::binary );
	file.seekg(0,ios::beg);
	cout<<"\n\nRecords in Phone Book:\n";
	while(file) {
		file.read((char *) this, sizeof(*this));
...
void pvt :: deleterec() {
	...
	cin>>bb;
	fstream file;
	file.open("private.txt",  ios::in | ios::binary );
	fstream fout;
	fout.open("tempfile.txt",ios::out | ios::binary);
	int i=0,j=0,found=0;
	file.read((char *) this, sizeof(*this));
	...
	if(i-j<=1) {
		...
		file.close();
		remove("private.txt");
		rename("tempfile.txt","private.txt");
		if(j<i)
			cout<<"Record successfully deleted!!"<<endl;
		...
		file.close();
		remove("private.txt");
		rename("tempfileph.txt","private.txt");
		if(j<i)cout<<"\n\nRECORD DELETED\n\n"<<endl;
		else cout<<"Record NOT found!!"<<endl;
	}
}